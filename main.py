...
if __name__ == '__main__':
    main()
'''reply_keyboard = [['/data_today', '/time_now'],
                  ['/help', '/close_keyboard']]
markup = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=False)


def help(update, context):
    update.message.reply_text("/time_now, время сейчас, с точностью до секунды!"
...
def data_today(update, context):
    update.message.reply_text(datetime.datetime.now().strftime('%d.%m.%Y'))
...
def timer(update, context):
    ...
    try:
        ...
        if update.message.text[0] == 'с':
            ...
            context.chat_data['job'] = new_job
            update.message.reply_text(f'Вернусь через {due} секунд')
        elif update.message.text[0] == 'ч':
            due = int(context.args[0] * 60 * 60)
            if 'job' in context.chat_data:
                old_job = context.chat_data['job']
                old_job.schedule_removal()
            new_job = context.job_queue.run_once(task, due, context=chat_id)
...
def main():
    ...
    dp.add_handler(CommandHandler("data_today", data_today))
    dp.add_handler(CommandHandler("help", help))
    dp.add_handler(CommandHandler("set", timer,
                                  pass_args=True,
                                  pass_job_queue=True,
                                  pass_chat_data=True))
